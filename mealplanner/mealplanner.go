   ...
   handler.handleRequest(c.lid,
      ...
      switch method {
         ...
         case "DELETE":
		      ...
		      iter := c.RunUnique(query, func()interface{}{return &Menu{}})
		      for !iter.End() {
               mkey, menuPtr, err := iter.Next()
               check(err)
               menu := menuPtr.(*Menu)
...
// Interator for looping over datastore query results
type QueryIter struct {
   iter *datastore.Iterator
   dst interface{}
   factory func() interface{}
   err os.Error
   fetch func() (*datastore.Key, interface{}, os.Error)
}

// Returns true iff QueryIter is at the end
func (self *QueryIter) End() bool {
   if self.fetch == nil {
      self.prefetch()
   }
   return self.err == datastore.Done
}

// prefetch the next result, sorting a functor to retreive them
//  and saving the err so End() can return true or not
func (self *QueryIter) prefetch() {
   dst := self.dst
   if self.factory != nil {
      dst = self.factory()
   }
   key, err := self.iter.Next(dst)
   // save a functor that can return the results
   self.err, self.fetch = err, func() (*datastore.Key, interface{}, os.Error) {
      return key, dst, err
   }
}

// Returns the next key, object, and error, and prefetches so we'll know if we're at
//  the end
func (self *QueryIter) Next() (*datastore.Key, interface{}, os.Error) {
   // if we've prefetched, call the functor and clear the prefetched data
   if self.fetch != nil {
      fetch := self.fetch
      self.fetch = nil
      return fetch()
   }
   // otherwise, retreive the next result
   dst := self.dst
   if self.factory != nil {
      dst = self.factory()
   }
...
// Run the query and return our QueryIter loading into dst
func (self *context) Run(query *datastore.Query, dst interface{}) *QueryIter {
   iter := query.Run(self.c)
   return &QueryIter{iter, dst, nil, nil, nil}
}

// Run the query and return our QueryIter loading into destinations generated by factory
func (self *context) RunUnique(query *datastore.Query, factory func()interface{}) *QueryIter {
   iter := query.Run(self.c)
   return &QueryIter{iter, nil, factory, nil, nil}
}
...
func (self *dataHandler) getAll(ancestor *datastore.Key) {
   ...
   iter := self.RunUnique(query, func()interface{}{return self.factory()})
   for !iter.End() {
      key, itemPtr, err := iter.Next()
      check(err)
      item := itemPtr.(Ided)
